...
public abstract class JobStoreSupport implements JobStore, Constants {
    ...
    protected RecoverMisfiredJobsResult recoverMisfiredJobs(
        ...
        if (hasMoreMisfiredTriggers) {
            ...
            for (TriggerKey triggerKey : misfiredTriggers) {
                FireUtil.addFire("missed " + triggerKey.getName() + " " + TimeUtil.getTimeNow());
            }
     ...
     * </p>
     *
     * @see #resumeTriggerGroup(Connection, GroupMatcher)
     */
    @SuppressWarnings("unchecked")
     ...
     *
     * @see #resumeAll()
     * @see #pauseTriggerGroup(Connection, GroupMatcher)
     */
    ...
    protected List<OperableTrigger> acquireNextTrigger(Connection conn, long noLaterThan, int maxCount, long timeWindow)
        ...
        do {
            ...
            try {
                ...
                System.out.println("acquireNextTrigger - now:" + System.currentTimeMillis() + " noLaterThan:" + (noLaterThan + timeWindow) + " getMisfireTime():" + getMisfireTime() + " maxCount:" + maxCount + " SchedulerNameLiteral" + ((StdJDBCDelegate) driverDelegate).getSchedulerNameLiteral());

                System.out.println(((StdJDBCDelegate) driverDelegate).getSchedulerNameLiteral());
                List<TriggerKey> keys = driverDelegate.selectTriggerToAcquire(conn, noLaterThan + timeWindow, getMisfireTime(), maxCount);
                ...
                for (TriggerKey triggerKey : keys) {
                    ...
                    System.out.println("更新执行状态 WAITING to ACQUIRED ，triggerKey：" + triggerKey + " time:" + TimeUtil.getTimeNow());
    ...
    class MisfireHandler extends Thread {
        ...
        public void run() {
            ...
            //只要调度器不停止，就会一直跑，正常情况下会一直跑
            while (!shutdown) {

                long sTime = System.currentTimeMillis();

                //查询next_fire_time < (当前时间的-失火时间)、状态是代执行的触发器
                RecoverMisfiredJobsResult recoverMisfiredJobsResult = manage();

                if (recoverMisfiredJobsResult.getProcessedMisfiredTriggerCount() > 0) {

                    System.out.println("出现了失火的触发器");
                    //改变时间调度信号，告知调度线程，在你睡觉的这段时间有任务要执行，并唤醒sigLock; signaled = true; signaledNextFireTime = candidateNewNextFireTime;
